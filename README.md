...
该fork是基于WeFlow 的一次的二创，融合项目原有的风格添加**AI智能回复**功能，实时智能回复聊天内容。
...
BotPage.tsx (前端界面) ──IPC──→ botBridgeService.ts (Node.js 桥接) ──WebSocket──→ Python AI 引擎
│ │
WCDB 数据库轮询 Claude/OpenAI 推理
配置持久化 发送后端执行


- **BotPage.tsx (前端界面)**：提供控制面板、配置、日志、统计等交互，通过 Electron IPC 与桥接服务通信。
...
| 功能 | 说明 |
|------|------|
| 启动/暂停/停止 | BotPage 控制面板操作，状态实时同步 |
| 三种监听模式 | 全部联系人 / 仅白名单 / 黑名单排除 |
| 事件日志 | 实时显示收到消息、发送回复、错误、心跳等 10 种事件 |
| 配置面板 | AI 模型、回复延迟（1-4秒）、冷却时间（12秒）、轮询间隔 |
| 联系人选择器 | 双列搜索筛选，支持头像显示，批量添加/移除 |
...
- **人类行为模拟**：随机延迟、分段发送、联系人间冷却
- **可定制人格**：通过 `.md` 文件定义对话风格，支持多套人格切换
