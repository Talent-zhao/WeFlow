...
import { BotBridgeService, resolveBotDir } from './services/botBridgeService'
...
import { registerInsightHandlers } from './handlers/insight'
...
let botBridgeService: BotBridgeService | null = null
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('chat:updateMessage', async (_, sessionId: string, localId: number, createTime: number, newContent: string) => {
    if (typeof newContent !== 'string' || newContent.length > 500 * 1024) {
      return { success: false, error: '消息内容过长' }
    }
  ...
  ipcMain.handle('window:openImageViewerWindow', async (_, imagePath: string, liveVideoPath?: string) => {
    ...
    if (imagePath.startsWith('data:')) {
      ...
      const rawExt = meta.split('/')[1]?.split(';')[0] || 'jpg'
  ...
  ipcMain.handle('http:start', async (_, port?: number, host?: string) => {
    ...
    const bindPort = typeof port === 'number' && port > 0 && port < 65536 ? port : 5031
    return httpService.start(bindPort, bindHost)
  })
  ...
  ipcMain.handle('bot:start', async () => {
    return botBridgeService!.start()
  })
  ipcMain.handle('bot:stop', async () => {
    return botBridgeService!.stop()
  })
  ipcMain.handle('bot:pause', async () => {
    return botBridgeService!.pause()
  })
  ipcMain.handle('bot:resume', async () => {
    return botBridgeService!.resume()
  })
  ipcMain.handle('bot:getStatus', async () => {
    return botBridgeService!.getStatus()
  })
  ipcMain.handle('bot:getConfig', async () => {
    return botBridgeService!.getConfig()
  })
  ipcMain.handle('bot:setConfig', async (_, key: string, value: any) => {
    return botBridgeService!.setConfig(key, value)
  })
  ipcMain.handle('bot:getStats', async () => {
    return botBridgeService!.getStats()
  })
  ipcMain.handle('bot:getLogs', async (_, limit?: number) => {
    return botBridgeService!.getLogs(limit)
  })
  ipcMain.handle('bot:clearLogs', async () => {
    return botBridgeService!.clearLogs()
  })
  ipcMain.handle('bot:testConnection', async () => {
    return botBridgeService!.testConnection()
  })
  ipcMain.handle('bot:sendMessage', async (_, contact: string, message: string) => {
    return botBridgeService!.sendMessage(contact, message)
  })

  // ── Persona management ──

  function getBuiltinPersonaDir(): string {
    return join(resolveBotDir(), 'personas')
  }

  function getUserPersonaDir(): string {
    return join(app.getPath('userData'), 'personas')
  }

  const isValidPersonaName = (name: string): boolean => {
    const trimmed = String(name || '').trim()
    if (!trimmed) return false
    if (trimmed.length > 200) return false
    if (trimmed.includes('..') || trimmed.includes('/') || trimmed.includes('\\')) return false
    if (/^\./.test(trimmed)) return false
    return /^[a-zA-Z0-9一-鿿_\-. ]+$/.test(trimmed)
  }

  ipcMain.handle('persona:list', async () => {
    const personas: { name: string; source: 'builtin' | 'user' }[] = []
    const seen = new Set<string>()

    // User personas first (take priority)
    const userDir = getUserPersonaDir()
    if (existsSync(userDir)) {
      for (const entry of readdirSync(userDir)) {
        if (entry.endsWith('.md')) {
          const name = entry.slice(0, -3)
          seen.add(name)
          personas.push({ name, source: 'user' })
        }
      }
    }

    // Built-in personas
    const builtinDir = getBuiltinPersonaDir()
    if (existsSync(builtinDir)) {
      for (const entry of readdirSync(builtinDir)) {
        if (entry.endsWith('.md')) {
          const name = entry.slice(0, -3)
          if (!seen.has(name)) {
            personas.push({ name, source: 'builtin' })
          }
        }
      }
    }
  ...
  ipcMain.handle('persona:read', async (_, name: string) => {
    if (!isValidPersonaName(name)) {
      throw new Error('Invalid persona name')
    }
    // User personas take priority over built-in
    const userPath = join(getUserPersonaDir(), `${name}.md`)
    if (existsSync(userPath)) {
      return { content: readFileSync(userPath, 'utf-8'), source: 'user' as const }
    }

    const builtinPath = join(getBuiltinPersonaDir(), `${name}.md`)
    if (existsSync(builtinPath)) {
      return { content: readFileSync(builtinPath, 'utf-8'), source: 'builtin' as const }
    }

    throw new Error(`Persona not found: ${name}`)
  })

  ipcMain.handle('persona:write', async (_, name: string, content: string) => {
    if (!isValidPersonaName(name)) {
      return { success: false, error: 'Invalid persona name' }
    }
    if (typeof content !== 'string' || content.length > 5 * 1024 * 1024) {
      return { success: false, error: 'Persona content is too large' }
    }
    const userDir = getUserPersonaDir()
    if (!existsSync(userDir)) {
  ...
  ipcMain.handle('persona:delete', async (_, name: string) => {
    if (!isValidPersonaName(name)) {
      return { success: false, error: 'Invalid persona name' }
    }
    const userPath = join(getUserPersonaDir(), `${name}.md`)
...
app.whenReady().then(async () => {
  ...
  botBridgeService = new BotBridgeService()
  applyAutoUpdateChannel('startup')
  ...
  botBridgeService?.setMainWindow(mainWindow)
...
const shutdownAppServices = async (): Promise<void> => {
  ...
  shutdownPromise = (async () => {
    ...
    // 停止智能回复机器人，释放子进程
    try { await botBridgeService?.dispose() } catch {}
