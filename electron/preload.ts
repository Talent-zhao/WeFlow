...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  key: {
    ...
    onDbKeyStatus: (callback: (payload: { message: string; level: number }) => void) => {
      const listener = (_: any, payload: { message: string; level: number }) => callback(payload)
    ...
    onImageKeyStatus: (callback: (payload: { message: string }) => void) => {
      const listener = (_: any, payload: { message: string }) => callback(payload)
  ...
  analytics: {
    ...
    onProgress: (callback: (payload: { status: string; progress: number }) => void) => {
      const listener = (_: any, payload: { status: string; progress: number }) => callback(payload)
  ...
  whisper: {
    ...
    onDownloadProgress: (callback: (payload: { modelName: string; downloadedBytes: number; totalBytes?: number; percent?: number }) => void) => {
      const listener = (_: any, payload: { modelName: string; downloadedBytes: number; totalBytes?: number; percent?: number }) => callback(payload)
  ...
  bot: {
    ...
    setConfig: (key: string, value: any) => ipcRenderer.invoke('bot:setConfig', key, value),
    getStats: () => ipcRenderer.invoke('bot:getStats'),
    getLogs: (limit?: number) => ipcRenderer.invoke('bot:getLogs', limit),
    clearLogs: () => ipcRenderer.invoke('bot:clearLogs'),
    testConnection: () => ipcRenderer.invoke('bot:testConnection'),
    sendMessage: (contact: string, message: string) =>
      ipcRenderer.invoke('bot:sendMessage', contact, message),
  ...
  persona: {
    ...
    read: (name: string) => ipcRenderer.invoke('persona:read', name),
    write: (name: string, content: string) => ipcRenderer.invoke('persona:write', name, content),
    delete: (name: string) => ipcRenderer.invoke('persona:delete', name),
  }
})
