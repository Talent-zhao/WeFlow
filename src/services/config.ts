...
export async function getBotPersona(): Promise<string> {
  const value = await config.get(CONFIG_KEYS.BOT_PERSONA)
  return typeof value === 'string' && value ? value : 'zhaoyoucai'
}

export async function setBotPersona(persona: string): Promise<void> {
  await config.set(CONFIG_KEYS.BOT_PERSONA, persona || 'zhaoyoucai')
}
