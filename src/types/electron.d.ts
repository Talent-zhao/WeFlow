...
export interface GroupSummaryTriggerDayResult {
  success: boolean
  message: string
  generated: number
  skipped: number
...
export type ListenMode = 'all' | 'specific' | 'blacklist'
...
export interface BotEvent {
  type: 'status' | 'scan' | 'msg_received' | 'reply_sent' | 'error' |
        'info' | 'heartbeat' | 'chat_switch' | 'config_updated' | 'ocr_dump'
  ts?: number
  contact?: string
  text?: string
  message?: string
...
export interface ElectronAPI {
  ...
  bot: {
    ...
    getLogs: (limit?: number) => Promise<BotEvent[]>
  ...
  persona: {
    list: () => Promise<{ name: string; source: 'builtin' | 'user' }[]>
    read: (name: string) => Promise<{ content: string; source: 'builtin' | 'user' }>
    write: (name: string, content: string) => Promise<{ success: boolean }>
